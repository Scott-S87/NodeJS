...
```
## Objective
Install a legacy version of NodeJS from source code, which is available via GitHub.

## Summary
A Linux user is required to utilize an earlier version of NodeJS, in order to accomplish their development objectives. Most Linux repositories contain a current version of NodeJS, which a Package Manager application can easily query and install. One of the ways to obtain earlier NodeJS versions, is to obtain the source code, and install it manually.
<br>
<br>
This document illustrates the process obtaining and installing from source code. Most of the steps will contain a bullet summary, a field where the user can copy the commands to paste into their terminal, and a screenshot to further illustrate each step.  
